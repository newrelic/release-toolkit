    ...
    - name: anotherdep
      ...
      meta:
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err = app.Run(strings.Fields(fmt.Sprintf("rt --yaml %s %s generate-changes-file -git-root %s -markdown %s %s", yamlPath, tc.preCmdArgs, tDir, mdPath, tc.args)))
			if err != nil {
...
// As the generate-changes-file command populates hashes in the yaml output, we need to know them for test data.
