...
- This is in the past and should be preserved
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err = app.Run(strings.Fields(fmt.Sprintf("rt -changelog %s update-changelog -md %s %s", yamlPath, mdPath, tc.args)))
			if err != nil {
...
func TestRender_Fails_Without_Version(t *testing.T) {
	...
	err = app.Run([]string{
		"rt", "-changelog", yamlPath, "update-changelog", "-md", mdPath,
	})
...
`)
	...
	err = app.Run(
		[]string{
			"rt", "-changelog", yamlPath, "update-changelog", "-md", mdPath, "-version", "v1.2.4",
		})
	if err != nil {
		t.Fatal(err)
	}
